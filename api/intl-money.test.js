...
test('markup 70% then FX: R$72.90 → base R$123.93', () => {
  assert.equal(DEFAULT_INTL_MARKUP_PERCENT, 70);
  assert.equal(intlBaseBrl(72.9, 70), 123.93);
  assert.equal(applyFxAmount(123.93, FX.USD, 2), 24.18);
  assert.equal(applyFxAmount(123.93, FX.EUR, 2), 20.8);
  assert.equal(applyFxAmount(123.93, FX.SEK, 0), 232);
});

test('applyMarkupFxToProduct writes all currency fields from one BRL base', () => {
  const { product, changed } = applyMarkupFxToProduct(
    { price: 72.9, intlMarkupPercent: 70 },
    DEFAULT_INTL_CURRENCIES,
    FX
  );
  assert.equal(changed, true);
  assert.equal(product.intlBaseBrl, 123.93);
  assert.equal(product.priceUsd, 24.18);
  assert.equal(product.priceEur, 20.8);
  assert.equal(product.pricePln, 89.66);
  assert.equal(product.priceGbp, 17.87);
  assert.equal(product.priceSek, 232);
  assert.equal(product.priceNok, 224);
});

test('smartband stays distinct with same markup', () => {
  const watch = applyMarkupFxToProduct({ price: 72.9, intlMarkupPercent: 70 }, DEFAULT_INTL_CURRENCIES, FX).product;
  const band = applyMarkupFxToProduct({ price: 62.9, intlMarkupPercent: 70 }, DEFAULT_INTL_CURRENCIES, FX).product;
  assert.equal(watch.priceUsd, 24.18);
  assert.equal(band.priceUsd, 20.86);
...
test('applyMarkupFxToIntlProducts only touches INT market rows', () => {
  ...
  assert.equal(products[1].priceUsd, 24.18);
  assert.equal(products[1].intlMarkupPercent, 70);
});
