 ...
 * Cookies stf_pref_lang are host-only (not shared across .com / .com.br), so
 * cross-domain auto-redirects create ERR_TOO_MANY_REDIRECTS
 * (pt cookie on .com → .br, then en on .br → .com, forever).
 *
 * Cross-host is only via explicit language switcher / ?stf_lang=
 * (handled in the proxy). INT lens vs BR kit stay on their markets.
 *
...
export function localeRedirectTarget({ hostOrigin, pathname, search, br, preferred }) {
  ...
  // .com: only prefix /de|/pl|/fr|… within the same host; never send pt → .com.br
